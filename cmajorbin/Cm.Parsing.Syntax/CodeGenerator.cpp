...
#pragma warning(default: 4244)
    ...
    hppFormatter.WriteLine("#include <Cm.Parsing/Grammar.hpp>");
    hppFormatter.WriteLine("#include <Cm.Parsing/Keyword.hpp>");
    if (hppFileName != "StdLib.hpp")
    {
        cppFormatter.WriteLine("#include \"" + hppFileName + "\"");
    }
    cppFormatter.WriteLine("#include <Cm.Parsing/Action.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/Rule.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/ParsingDomain.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/Primitive.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/Composite.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/Nonterminal.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/Exception.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/StdLib.hpp>");
    cppFormatter.WriteLine("#include <Cm.Parsing/XmlLog.hpp>");
    ...
    for (NamespaceContentMapIt i = nsContent.begin(); i != e; ++i)
    {
        ...
        if (!content.UsingObjects().empty())
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                Cm::Parsing::CppObjectModel::UsingObject* subject = usingObject->Subject();
        ...
        cppFormatter.WriteLine("using namespace Cm::Parsing;");
        cppFormatter.WriteLine();
