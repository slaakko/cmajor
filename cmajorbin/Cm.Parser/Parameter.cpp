...
ParameterGrammar::ParameterGrammar(Cm::Parsing::ParsingDomain* parsingDomain_): Cm::Parsing::Grammar("ParameterGrammar", parsingDomain_->GetNamespaceScope("Cm.Parser"), parsingDomain_)
{
    SetOwner(0);
}

void ParameterGrammar::Parse(const char* start, const char* end, int fileIndex, const std::string& fileName, ParsingContext* ctx, Cm::Ast::Node* owner)
{
    ...
    stack.push(std::unique_ptr<Cm::Parsing::Object>(new ValueObject<Cm::Ast::Node*>(owner)));
