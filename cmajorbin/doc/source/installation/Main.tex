...
You may want to change the install location under \verb|C:\Program Files| directory in this case.

\item
The setup adds \verb|C:\Program Files\Cmajor\bin| directory (or whereever you installed it) to your systems
\textbf{PATH} environment variable, so the Cmajor programs can be executed from any directory from the
command prompt without specifying full paths.
...
and run \verb!Build|Rebuild Solution! command for the \verb|release| configuration.
...
\item
Select \verb|release| configuration from the configuration combo box
and run \verb!Build|Rebuild Solution! command for the \verb|C| backend and for the \verb|release| configuration.
...
Note: If you are updating from previous Cmajor version, it is important to issue the \textbf{rebuild} command (not just build command),
because System Library directories are not cleared when uninstalling Cmajor.
...
\item
cmc I2 error \textbf{library reference 'system.cml' not found}.
...
\item
Download, build and install Boost (\url{http://www.boost.org/}). You will need to build and install the filesystem library.
