...
and then go though the algorithms for checking, binding and comparing constraints.

\section{Concepts in Overload Resolution}

Concepts are used in the overload resolution process (chapter \ref{overloadresolution}) in three stages:
...
We may also say that concepts are used to \emph{exclude} an unwanted constrained function template from the list of matching functions,
if the constraint of the function template is \emph{not} satisfied. This is just the other side of the coin.
...
\item
The next stage is to \emph{bind} the constraints that are satisfied so that they form \emph{bound constraints}.

\item
The third stage is to \emph{compare} the bound constraints using the \emph{imply} relation.
...
The following algorithm is used for checking constraints:

\begin{algo} Check Constraint. Inputs: a container scope, a constraint represented as an abstract syntax tree node,
list of template parameters, list of template argument types.
...
It has also a type, \emph{resolvedType}, and a concept group symbol, \emph{resolvedConceptGroup}.
\item
...
\item
If $r$ is \textbf{false}, push \textbf{false} to the constraint check stack and return.
\end{enumerate}
\item
...
\item
If $c$ is \textbf{false}, push \textbf{false} to the constraint check stack and return.
\end{enumerate}
\item
...
\item
If $c$ is \textbf{true}, push \textbf{true} to the constraint check stack and return.
\item
Otherwise, call \verb|Accept| member function of $right$ with this constraint checking visitor. The result of visitation is left to the constraint check stack.
\end{enumerate}
\item
...
Lookup $conceptId$ from the concept repository (section \ref{conceptrepository}).
\item
If found, push \textbf{true} to the constraint check stack and return.
\item
Otherwise, instantiate concept $s$ with type arguments $a$ using algorithm \ref{instantiateconcept}.
Let $c$ be the instantiated concept.
\item
...
\item
If $resolvedType$ is not null, push \textbf{true} to the constraint check stack.
\item
Otherwise, push \textbf{false} to the constraint check stack.
\end{enumerate}
\item
...
\item
If a constructor is found, push \textbf{true} to the constraint check stack.
\item
Otherwise, push \textbf{false} to the constraint check stack.
\end{enumerate}
\item
...
\item
If a member function is found, push \textbf{true} to the constraint check stack.
\item
Otherwise, push \textbf{false} to the constraint check stack.
\end{enumerate}
\item
...
\item
If a function is found, push \textbf{true} to the constraint check stack.
\item
Otherwise, push \textbf{false} to the constraint check stack.
\end{enumerate}
\item
\verb|Visit(SameConstraintNode& sameConstraintNode)|:\\
If types $firstArgumentType$ and $secondArgumentType$ are same, push \textbf{true} to the constraint check stack.
Otherwise push \textbf{false} to the constraint check stack.
\item
\verb|Visit(DerivedConstraintNode& derivedConstraintNode)|:\\
If type $firstArgumentType$ is derived from the $secondArgumentType$, push \textbf{true} to the constraint check stack.
Otherwise push \textbf{false} to the constraint check stack.
\item
\verb|Visit(ConvertibleConstraintNode& convertibleConstraintNode)|:\\
If type $firstArgumentType$ is implicitly convertible to the $secondArgumentType$, push \textbf{true} to the constraint check stack.
Otherwise push \textbf{false} to the constraint check stack.
\item
\verb|Visit(ExplicitlyConvertibleConstraintNode& explicitlyConvertibleConstraintNode)|:\\
If type $firstArgumentType$ is explicitly convertible to the $secondArgumentType$, push \textbf{true} to the constraint check stack.
Otherwise push \textbf{false} to the constraint check stack.
\item
...
If $commonType$ is not null, install $commonType$ to the container scope, and push \textbf{true} to the constraint check stack,
otherwise push \textbf{false} to the constraint check stack.
\item
\verb|Visit(NonReferenceTypeConstraintNode& nonReferenceTypeConstraintNode)|:\\
If $firstArgumentType$ is not lvalue reference type and not rvalue reference type, push \textbf{true} to the constraint check stack,
otherwise push \textbf{false} to the constraint check stack.
\end{itemize}
\item
...
Inputs: a container scope, a concept symbol $s$, list of type arguments $a$.
...
\item
If $r$ is \textbf{true}, create an instantiated concept symbol for $s$ with type arguments $a$, and return it.
\item
Otherwise, return null.
\end{enumerate}
\end{algo}
...
When many constraints are satisfied they must be bound before they can be compared.
...
BoundNode
    BoundConstraint
        ...
        BoundTypeSatisfyConceptConstraint
        BoundTypeIsTypeConstraint
        BoundMultiParamConstraint
        BoundConcept
\end{verbatim}

The following algorithm is used to create bound constraints:

\begin{algo}\label{bindconstraint} Binding a Constraint. Inputs: list of template parameters, list of template arguments, a container scope,
a constraint represented as an abstract syntax tree node.
The algorithm returns a bound constraint.
\begin{enumerate}
\item
Create a constraint binding scope and set its parent to the container scope.
\item
Let $n$ be the number of template parameters. $n$ is also the number of template arguments.
\item
For $i$ = 1, \ldots, $n$:
\begin{enumerate}
\item
Let $p$ be $i$'th template parameter.
\item
Let $t$ be $i$'th template argument.
\item
Create a bound type parameter that maps the name of $p$ to type $t$, and install it to the constraint binding scope.
\end{enumerate}
\item
Create an instance of a constraint binder that is an abstract syntax tree visitor.
The constraint binder contains:
\begin{itemize}
\item
A container scope.
\item
A type $resolvedType$.
\item
A concept group $resolvedConceptGroup$.
\item
A concept symbol $conceptSymbol$.
\item
A stack of bound constraints.
\end{itemize}
\item
Pass constraint binding scope as a container scope to the constraint binder.
\item
Call \verb|Accept| member function of the constraint abstract syntax tree node with constraint binder.
\item
Return the result of visitation that is a bound constraint on top of the stack of bound constraints.
\item
The constraint binder overrides the following visitations points:
\begin{itemize}
\item
\verb|Visit(ConceptNode& conceptNode)|:\\
\begin{enumerate}
\item
Call \verb|Accept| member function of the concept identifier contained by the concept node.
\item
A concept group $resolvedConceptGroup$ is the result of visitation.
\item
Let $n$ be the number of type parameters of concept node.
\item
Get a concept symbol with $n$ type parameters from the $resolvedConceptGroup$.
\item
Create a \verb|BoundConcept| node with the concept symbol and push it to the stack of bound constraints.
\end{enumerate}
\item
\verb|Visit(DisjunctiveConstraintNode& disjunctiveConstraintNode)|:\\
\begin{enumerate}
\item
Call \verb|Accept| member function of the left constraint of the disjunctive constraint.
\item
Pop the bound left constraint from the stack of bound constraints.
\item
Call \verb|Accept| member function of the right constraint of the disjunctive constraint.
\item
Pop the bound right constraint from the stack of bound constraints.
\item
Create a |BoundDisjunctiveConstraint| with the left and right bound constraints and push it to the stack of bound constraints.
\end{enumerate}
\item
\verb|Visit(ConjunctiveConstraintNode& conjunctiveConstraintNode)|:\\
\begin{enumerate}
\item
Call \verb|Accept| member function of the left constraint of the conjunctive constraint.
\item
Pop the bound left constraint from the stack of bound constraints.
\item
Call \verb|Accept| member function of the right constraint of the conjunctive constraint.
\item
Pop the bound right constraint from the stack of bound constraints.
\item
Create a |BoundConjunctiveConstraint| with the left and right bound constraints and push it to the stack of bound constraints.
\end{enumerate}
\item
\verb|Visit(IdentifierNode& identifierNode)|:\\
\begin{enumerate}
\item
Set $resolvedType$ to null.
\item
Set $resolvedConceptGroup$ to null.
\item
Lookup a symbol with the name contained by the identifier node from the container scope.
\item
Let $s$ be the symbol found.
\item
If $s$ is a type symbol, set $resolvedType$ to $s$.
\item
Otherwise, if $s$ is a bound type parameter symbol, set $resolvedType$ to the mapped type of the bound type parameter.
\item
Otherwise, if $s$ is a concept group symbol, set $resolvedConceptGroup$ to $s$.
\item
Otherwise, report error.
\end{enumerate}
\item
...
\begin{enumerate}
\item
Lookup a symbol with the name contained by the dot node from the container scope of $resolvedType$.
\item
Let $s$ be the symbol found.
\item
If $s$ is a type symbol, set $resolvedType$ to $s$.
...
\item
\verb|Visit(BoolNode& boolNode)|:\\
Set $resolvedType$ to \verb|BoolTypeSymbol|.
\item
\verb|Visit(SByteNode& sbyteNode)|:\\
Set $resolvedType$ to \verb|SByteTypeSymbol|.
\item
\verb|Visit(ByteNode& byteNode)|:\\
Set $resolvedType$ to \verb|ByteTypeSymbol|.
\item
\verb|Visit(ShortNode& shortNode)|:\\
Set $resolvedType$ to \verb|ShortTypeSymbol|.
\item
\verb|Visit(UShortNode& ushortNode)|:\\
Set $resolvedType$ to \verb|UShortTypeSymbol|.
\item
\verb|Visit(IntNode& intNode)|:\\
Set $resolvedType$ to \verb|IntTypeSymbol|.
\item
\verb|Visit(UIntNode& uintNode)|:\\
Set $resolvedType$ to \verb|UIntTypeSymbol|.
\item
\verb|Visit(LongNode& longNode)|:\\
Set $resolvedType$ to \verb|LongTypeSymbol|.
\item
\verb|Visit(ULongNode& ulongNode)|:\\
Set $resolvedType$ to \verb|ULongTypeSymbol|.
\item
\verb|Visit(FloatNode& floatNode)|:\\
Set $resolvedType$ to \verb|FloatTypeSymbol|.
\item
\verb|Visit(DoubleNode& doubleNode)|:\\
Set $resolvedType$ to \verb|DoubleTypeSymbol|.
\item
\verb|Visit(CharNode& charNode)|:\\
Set $resolvedType$ to \verb|CharTypeSymbol|.
\item
\verb|Visit(WCharNode& wcharNode)|:\\
Set $resolvedType$ to \verb|WCharTypeSymbol|.
\item
\verb|Visit(UCharNode& ucharNode)|:\\
Set $resolvedType$ to \verb|UCharTypeSymbol|.
\item
\verb|Visit(VoidNode& voidNode)|:\\
Set $resolvedType$ to \verb|VoidTypeSymbol|.
\item
\verb|Visit(DerivedTypeExprNode& derivedTypeExprNode)|:\\
Set $resolvedType$ to a type resolved from derived type expression node using type resolver.
\item
\verb|Visit(IsConstraintNode& isConstraintNode)|:\\
\begin{enumerate}
\item
Call \verb|Accept| member function of the type expression contained by the isConstraintNode.
\item
Let $leftType$ be the $resolvedType$.
\item
Call \verb|Accept| member function of the concept or type name contained by the isConstraintNode.
\item
If $resolvedType$ is not null, create a \verb|BoundTypeIsTypeConstraint| with $leftType$ as the left type and $resolvedType$ as the right type,
and push it to the stack of bound constraints.
\item
Otherwise, if $resolvedConceptGroup$ is not null:
\begin{enumerate}
\item
Get concept symbol of one type parameter from the $resolvedConceptGroup$.
\item
Get an abstract syntax tree node $node$ that corresponds to the concept symbol from the symbol table.
\item
Call \verb|Accept| member function of $node$.
\item
Pop bound concept $b$ from the stack of bound constraints.
\item
Create a \verb|BoundTypeSatisfyConceptConstraint| with type $leftType$ and bound concept $b$, and push it to the stack of bound constraints.
\end{enumerate}
\end{enumerate}
\item
\verb|Visit(MultiParamConstraintNode& multiParamConstraintNode)|:\\
\begin{enumerate}
\item
Call \verb|Accept| member function of the concept identifier of the multiparameter constraint node.
\item
If $resolvedConceptGroup$ is not null:
\begin{enumerate}
\item
Let $n$ be the number of type parameters of the multiparameter constraint node.
\item
Get concept symbol of $n$ type parametedes from the $resolvedConceptGroup$.
\item
Get an abstract syntax tree node $node$ that corresponds to the concept symbol from the symbol table.
\item
Call \verb|Accept| member function of $node$.
...
Pop bound concept $b$ from the stack of bound constraints.
\item
Create a list of type arguments $a$.
\item
For $i$ = 1, \ldots, $n$:
\begin{enumerate}
\item
Call \verb|Accept| member function of the $i$'th type expression of the multiparameter constraint node.
\item
Add $resolvedType$ to $a$.
\end{enumerate}
\item
Create a \verb|BoundMultiParamConstraint| with bound concept $b$ and type arguments $a$, and push it to the stack of bound constraints.
\end{enumerate}
\end{enumerate}
\item
\verb|Visit(TypenameConstraintNode& typenameConstraintNode)|:\\
Create a \verb|BoundAtomicConstraintNode| and push it to the stack of bound constraints.
\item
\verb|Visit(ConstructorConstraintNode& constructorConstraintNode)|:\\
Create a \verb|BoundAtomicConstraintNode| and push it to the stack of bound constraints.
\item
...
\begin{enumerate}
\item
If $A$ is \verb|BoundAtomicConstraint| return \textbf{true}.
\item
