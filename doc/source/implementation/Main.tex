...
The name of a symbol is either an empty string, or consists of a Latin letter or underscore followed
by any number of Latin letters, underscores and decimal digits.
...
\subsection{Opening and Closing Container Symbols}
...
\begin{algo} Opening a Namespace. The algorithm sets the currently open container symbol.
\begin{enumerate}
\item
Let $n$ be a possibly qualified namespace name.
\item
If $n$ is an empty string, push the currently open container symbol to the stack of open container symbols,
and then set the global namespace symbol as the currently open container symbol.
\item
...
\item
If $s$ is a namespace symbol, push the currently open container symbol to the stack of open container symbols,
and then set $s$ as the currently open container symbol. Otherwise if $s$ is not a namespace symbol, throw an exception.
\item
Otherwise $s$ is null, so use algorithm \ref{createns} to create a namespace to the container scope of the currently open container symbol,
and open it by pushing the currently open container symbol to the stack of open container symbols,
and then set the created namespace symbol as the currently open container symbol.
\end{enumerate}
\end{algo}
...
\begin{enumerate}
\item
First basic type symbols like \emph{BoolTypeSymbol} and \emph{IntTypeSymbol} are inserted to the global namespace
