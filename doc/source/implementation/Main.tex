...
\end{itemize}
\item
Chapter 12 describes the operation of the expression binder.
\item
Chapter 13 shows various statement binders.
\item
Chapter 14 describes how the main binder binds classes and functions.
\item
Chapter 15 discusses the compilation of templates.
\end{itemize}
\item
...
\begin{enumerate}
\item
Check access from the current function being compiled to the constant symbol.
\item
...
\begin{enumerate}
\item
Check access from the current function being compiled to the member variable symbol.
\item
...
\begin{enumerate}
\item
Call algorithm \ref{maketemplatetype} to make a class template specialization with the type of $subject$ and $typeArguments$. Let $type$ be the type returned.
\item
...
\chapter{Binding Classes and Functions}
...
\end{itemize}
\item
If the class is virtual, the virtual function table pointer is initialized.
\item
...
Receive statement.
\item
If the class is virtual, the virtual function table pointer is set to correspond to the class being destroyed.
\end{enumerate}
...
If the user has not written a destructor for a class and that class is virtual, has a nontrivial member variable destructor,
...
\item
If the class is virtual, the virtual function pointer is set to correspond this class type.
\item
...
and finally generating bound classes and functions and appending them to the current bound compile unit.

\section{Function Templates}

\subsection{Function Template Repository}

A function template key contains the primary function template symbol and list of template argument symbols.
...
\begin{algo}\label{getfunctiontemplatefromrepository} Get Function Template From Repository. Inputs: a function template key.
\begin{enumerate}
\item
If a function template intance for the function template key is found from the repository, return it.
\item
Otherwise, return null.
\end{enumerate}
\end{algo}

\begin{algo}\label{addfunctiontemplatetorepository} Add Function Template To Repository. Inputs: a function template key, a function template instance.
\begin{enumerate}
\item
Add $<key, instance>$ pair to the mappings.
\end{enumerate}
\end{algo}

\subsection{Binding Type Parameters}

\begin{algo}\label{bindtypeparam} Bind Type Parameters. Inputs: a primary function template, a function template instance, list of template arguments.
\begin{enumerate}
\item
...
\item
Create a \verb|BoundTypeParameterSymbol| that maps name of $typeParam$ to $templateArg$, and add it to the function template instance using algorithm
\ref{addsymboltocontainer}, so that the type resolver can find it from the scope of the function template instance (algorithm \ref{resolvesymboltype}).
\end{enumerate}
\end{enumerate}
\end{algo}

\subsection{Instantiation of a Function Template}
...
\begin{enumerate}
\item
Create a function template key, $key$, from the primary function template symbol and list of template argument symbols.
\item
Search $key$ from the function template repository (algorithm \ref{getfunctiontemplatefromrepository}). Let $symbol$ be the symbol found.
\item
