...
The first three rules for the basis. They tell us that a class of palindromes includes the strings $\epsilon$, $0$, and $1$.
None of the right sides of these rules contains a variable, which is why they for a basis for the definition.
...
Conside a nonterminal $A$ with two productions $$A \rightarrow A\alpha \,|\, \beta$$
...
A rule may have one synthesized attribute whose type is denoned by a colon and a name of a C++ type after
...
The \verb|cmpg| reads grammar definitions and constructs an internal representation for them.
...
A \emph{closure} node is a unary parsing node whose subtree represents expression $\eta$.
...
A \emph{positive} node is a unary parsing node whose subtree represents expression $\eta$.
...
An \emph{optional} node is a unary parsing node whose subtree represents expression $\eta$.
...
\item
The primitive expressions defined using the extended context-free notation are:
