...
Each function repository matches its \emph{function signatures} with the group name (definition \ref{groupname}),
arity (definition \ref{arity}) and argument information of searched functions.
...
If later in compilation of the compile unit, the same signature is found again, the cached function is returned.
...
\section{Collecting Viable Functions from Function Repositories}
...
\begin{itemize}
\item
argument category (\verb|rvalue| or \verb|lvalue|),
\item
...
\textbf{Signature}& \textbf{Condition}& \textbf{Function Symbol}\\
\hline
\verb|@constructor(A*)|& $A$ is an array type& \verb|DefaultCtor(A)|\\
\verb|@constructor(A*, A)|& $A$ is an array type& \verb|CopyCtor(A)|\\
\verb|@constructor(A*, const A&)|& $A$ is an array type& \verb|CopyCtor(A)|\\
\verb|operator=(A*, A)|& $A$ is an array type& \verb|CopyAssignment(A)|\\
\verb|operator=(A*, const A&)|& $A$ is an array type& \verb|CopyAssignment(A)|\\
