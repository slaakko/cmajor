...
public:
    ...
    {
        context.value = new ConceptNode(span, context.fromconceptName);
    }
...
public:
    ...
    void A0Action(const char* matchBegin, const char* matchEnd, const Span& span, const std::string& fileName, bool& pass)
    {
        context.value = new AxiomStatementNode(span, context.fromExpression);
    }
...
public:
    ...
    virtual void Link()
    {
        ...
        Cm::Parsing::NonterminalParser* identifierNonterminalParser = GetNonterminal("Identifier");
        identifierNonterminalParser->SetPostCall(new Cm::Parsing::MemberPostCall<TypenameConstraintRule>(this, &TypenameConstraintRule::PostIdentifier));
    }
    void A0Action(const char* matchBegin, const char* matchEnd, const Span& span, const std::string& fileName, bool& pass)
    {
        context.value = new TypenameConstraintNode(span, context.fromIdentifier);
    }
    void PostIdentifier(Cm::Parsing::ObjectStack& stack, bool matched)
    {
        if (matched)
...
public:
    ...
    void A1Action(const char* matchBegin, const char* matchEnd, const Span& span, const std::string& fileName, bool& pass)
    {
        ...
        if (pass) context.value = new ConstructorConstraintNode(span);
    }
    void PostIdentifier(Cm::Parsing::ObjectStack& stack, bool matched)
    {
        if (matched)
...
public:
    ...
    void A0Action(const char* matchBegin, const char* matchEnd, const Span& span, const std::string& fileName, bool& pass)
    {
        context.value = new DestructorConstraintNode(span);
    }
    void A1Action(const char* matchBegin, const char* matchEnd, const Span& span, const std::string& fileName, bool& pass)
    {
        context.id.reset(context.fromIdentifier);
...
private:
    struct Context
    {
        Context(): ctx(), value(), fromAtomicConstraintExpr(), fromConstraintExpr() {}
        ParsingContext* ctx;
        Cm::Ast::ConstraintNode* value;
